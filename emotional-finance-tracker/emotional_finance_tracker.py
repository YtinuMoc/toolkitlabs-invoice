#!/usr/bin/env python3
"""BodegaLabs Emotional Finance Tracker — clone of bodegalaabs.gumroad.com/l/emotional-finance-tracker (1,253 sales · 39 ratings · 5★ · $8+)."""
import csv
import sys
...
def load_budget_categories(path):
    return load_csv(path, ["category", "budget"], ["budget"])


def summarize_budget(categories_path, expenses_path, income_path=None):
    """jordilabs/budget-planner: category budgets + auto remaining budget indicators."""
    categories = load_budget_categories(categories_path)
    expenses = load_expenses(expenses_path)
    income_rows = load_income(income_path) if income_path else []
    ...
    total_income = sum(i["amount"] for i in income_rows)

    print("=== MONTHLY BUDGET DASHBOARD (jordilabs/budget-planner shape) ===")
    print("  Organize by category · auto remaining budget · stay-within-budget indicators.")
    if total_income:
        print(f"  Monthly income:      {fmt_money(total_income)}")
    print(f"  Total budget:        {fmt_money(total_budget)}")
    print(f"  Total spent:         {fmt_money(total_spent)}")
    print(f"  Remaining:           {fmt_money(total_budget - total_spent)}")
    print()
    print(f"  {'Category':<22} {'Budget':>10} {'Spent':>10} {'Left':>10} {'Used':>6}  Status")
    over_count = 0
    for cat in categories:
        ...
        if pct > 100:
            ...
            status = "✓ ok"
        print(f"  {name:<22} {fmt_money(budget):>10} {fmt_money(spent):>10} {fmt_money(remaining):>10} {pct:5.0f}%  {status}")
    print()
    print(f"  Categories over budget: {over_count}")
    print("  Guide: start-here.md · budget-categories-sample.csv · expenses-sample.csv")
...
def summarize_dashboard(income_path, expense_path, accounts_path, goals_path, debts_path, subs_path):
    ...
    print("=== MONTHLY BUDGET OVERVIEW (jordilabs/budget-planner clone) ===")
...
def load_wishlist(path):
    return load_csv(path, ["item", "priority", "price", "goal_link", "notes"], ["price"])


def summarize_wishlist(wishlist_path, goals_path):
    """bodegalaabs/emotional-finance-tracker: wishlist vs savings goals — impulse check."""
    items = load_wishlist(wishlist_path)
    goals = load_goals(goals_path)
    goal_map = {g["goal"]: g for g in goals}
    priority_rank = {"high": 0, "medium": 1, "low": 2}

    print("=== WISHLIST IMPULSE CHECK (bodegalaabs/emotional-finance-tracker shape) ===")
    print("  Rank desired purchases against savings goals — stop impulse spending.")
    print()
    for item in sorted(items, key=lambda x: (priority_rank.get(x["priority"].lower(), 9), -x["price"])):
        goal = item.get("goal_link", "").strip()
        if not goal:
            flag = "⚠ no goal linked — impulse risk"
            funded = None
        elif goal in goal_map:
            g = goal_map[goal]
            funded = (g["current"] / g["target"] * 100) if g["target"] else 0
            flag = f"→ {goal} ({funded:.0f}% funded)"
        else:
            funded = None
            flag = f"→ {goal} (goal not found)"
        print(f"  {item['item']:<24} {fmt_money(item['price'])}   {flag}")
        if item.get("notes"):
            print(f"    note: {item['notes']}")
    print()
    unlinked = [i for i in items if not i.get("goal_link", "").strip()]
    if unlinked:
        at_risk = sum(i["price"] for i in unlinked)
        print(f"  Items without goal:    {len(unlinked)} · ${at_risk:,.2f} at impulse risk")
    print("  Guide: wishlist-sample.csv · goals-sample.csv")
...
def main():
    ...
    if "--merge" in args:
        ...
        return
    if not args:
        print("Usage: emotional_finance_tracker.py <income> <expenses> <accounts> <goals> <debts> <subscriptions>")
        print("       emotional_finance_tracker.py --budget <categories> <expenses> [income]")
        print("       emotional_finance_tracker.py --take-home <income> <expenses>")
        print("       emotional_finance_tracker.py --wishlist <wishlist.csv> <goals.csv>")
        print("       emotional_finance_tracker.py --quarterly-tax <income> <expenses>")
        sys.exit(1)
    if args[0] == "--budget":
        income_path = args[3] if len(args) > 3 else None
        summarize_budget(args[1], args[2], income_path)
    elif args[0] == "--take-home":
        reserve = float(args[3]) if len(args) > 3 else 28.0
        summarize_take_home(args[1], args[2], reserve)
    elif args[0] == "--wishlist":
        summarize_wishlist(args[1], args[2])
