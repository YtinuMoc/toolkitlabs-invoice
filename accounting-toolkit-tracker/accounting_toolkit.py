#!/usr/bin/env python3
"""Accounting Toolkit Tracker — clone of theaccountantguy.gumroad.com/l/notionaccountingtoolkit (92 sales)."""
import csv
import sys
...
def summarize_dashboard(income_path, expense_path, accounts_path, goals_path, debts_path, subs_path):
    ...
    print("=== ACCOUNTING TOOLKIT (theaccountantguy clone) ===")
...
def main():
    ...
    if "--merge" in args:
        ...
        return
    if not args:
        print("Usage: accounting_toolkit.py <income> <expenses> <accounts> <goals> <debts> <subscriptions>")
        print("       accounting_toolkit.py --take-home <income> <expenses>")
        print("       accounting_toolkit.py --late-payment <invoices.csv>")
        print("       accounting_toolkit.py --quarterly-tax <income> <expenses>")
        sys.exit(1)
    if args[0] == "--take-home":
