...
Clone target: [hyeukiyo Notion Smart Financial (133 sales · 5 ratings · 5★)](https://hyeukiyo.gumroad.com/l/Fimgntemp).
...
```bash
python3 smart_financial_tracker.py --merge gumroad.csv stripe.csv paypal.csv -o ledger-merged.csv
```

Output columns: `date`, `platform`, `transaction_id`, `description`, `amount`, `fee`, `net`, `buyer`.

Import `ledger-merged.csv` into your income log — same workflow as hyeukiyo's connected income, expense, budget, subscription, and bill views.

## Pair with

- [net-income-guide.md](net-income-guide.md) — faisalmq/5797 safe-to-spend visibility (run357)
...
Full bundle: [Smart Financial Tracker landing](https://ytinumoc.github.io/toolkitlabs-invoice/smart-financial-tracker/).
